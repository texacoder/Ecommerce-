    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="border border-black/15 dark:border-white/20 rounded px-3 py-1.5 text-sm bg-transparent w-64"
        />
        <button onClick={load} className="text-sm underline">
          Search
        </button>
        <select value={filter} onChange={(e) => setFilter(e.target.value)} className="border border-black/15 dark:border-white/20 rounded px-2 py-1.5 text-sm bg-transparent ml-auto">
      ...
      <div className="border border-black/10 dark:border-white/10 rounded-lg overflow-x-auto">
            ...
            {rows.map((r) => (
              <Fragment key={r.productId}>
                <tr className="border-t border-black/5 dark:border-white/10">
                  ...
                  <td className="p-3 text-black/50 dark:text-white/50">{r.category ?? "—"}</td>
                  <td className="p-3">
                ...
                {r.variants.map((v) => (
                  <tr key={v.variantId} className="border-t border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02]">
                    <td className="p-3 pl-8 text-black/70 dark:text-white/70">↳ {v.name}</td>
        ...
        {rows.length === 0 && <p className="p-6 text-center text-black/50 dark:text-white/50">No matching products.</p>}
      </div>
    </div>
  );
}
...
function StockBadge({ lowStock, outOfStock }: { lowStock: boolean; outOfStock: boolean }) {
  if (outOfStock) return <span className="text-xs font-semibold text-rose-600">Out of stock</span>;
  if (lowStock) return <span className="text-xs font-semibold text-amber-600">Low stock</span>;
