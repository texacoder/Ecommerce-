          ...
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-t border-black/5 dark:border-white/10">
                ...
                <td className="p-3">
                  <span className={c.status === "ACTIVE" ? "text-emerald-600" : "text-rose-600"}>{c.status}</span>
                </td>
                <td className="p-3 text-black/50 dark:text-white/50">{formatDate(c.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {customers.length === 0 && <p className="p-6 text-center text-black/50 dark:text-white/50">No customers found.</p>}
      </div>
    </div>
  );
}
